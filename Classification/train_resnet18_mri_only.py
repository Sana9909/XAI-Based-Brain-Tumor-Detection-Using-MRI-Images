...
class MRIOnlyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return torch.tensor(img, dtype=torch.float32), int(row['label'])
...
def final_evaluation(
    ...
    save_path = Path(checkpoint_dir) / "confusion_matrix_mri_only.png"
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_acc > best_acc:
            ...
            torch.save(
                model.state_dict(),
                str(ckpt_dir / "best_resnet18_mri_only.pth")
            )
