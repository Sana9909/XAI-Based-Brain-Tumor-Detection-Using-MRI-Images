...
class MaskOnlyDataset(Dataset):
    ...
    def __init__(self, df, transforms=None, img_size=224):
    ...
    def __getitem__(self, idx):
        ...
        return torch.tensor(mask, dtype=torch.float32), int(row['label'])
...
def make_resnet18_1ch(num_classes, pretrained=True):
...
def final_evaluation(
    ...
    save_path = Path(checkpoint_dir) / "confusion_matrix_mask_only.png"
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_acc > best_acc:
            ...
            torch.save(
                model.state_dict(),
                str(ckpt_dir / "best_resnet18_mask_only.pth")
            )
