...
from Decoder_methods import DecoderBlock, CBE
...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        for i_layer in range(self.num_layers - 1):
            enc_idx  = self.num_layers - 2 - i_layer      # 2, 1, 0
            in_dim   = int(embed_dim * 2 ** (enc_idx + 1)) # 768, 384, 192
            in_res   = (
                patches_resolution[0] // (2 ** (enc_idx + 1)),
        ...
        if self.final_upsample == "expand_first":
            ...
            self.output = nn.Conv2d(
                in_channels=embed_dim, out_channels=self.num_classes,
